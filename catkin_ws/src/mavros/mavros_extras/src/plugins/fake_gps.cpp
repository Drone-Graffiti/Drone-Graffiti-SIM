...
private:
	 ...
	 */
	void send_fake_gps(const ros::Time &stamp, const Eigen::Vector3d &ecef_offset, const geometry_msgs::Pose &pose) {
		...
		double vel_x = ((pose.position.y - old_pose.position.y) / (stamp.toSec() - old_stamp)) * 1e2;
		double vel_y = ((pose.position.x - old_pose.position.x) / (stamp.toSec() - old_stamp)) * 1e2;
		double vel_z = ((pose.position.z - old_pose.position.z) / (stamp.toSec() - old_stamp)) * 1e2;
		
		//ROS_INFO(">>>!!!!>>>> %f, %f", vel_x, vel_y);

		// compute course over ground
		double cog = atan2(vel_y, vel_x);
		//if (vel.x() == 0 && vel.y() == 0) {
		//	cog = 0;
		//}
		//else {
		//	cog = atan2(vel.x(), vel.y());
			
	//	}
		//ROS_INFO(">>>!>>>> %f, %f", vel.x(), vel.y());
		...
		fix.vel = sqrt(vel_x*vel_x + vel_y*vel_y);	// [cm/s]
		fix.vn = vel_x;			// [cm/s]
		fix.ve = vel_y;			// [cm/s]
		fix.vd = -vel_z;			// [cm/s]
...
//		send_fake_gps(req->header.stamp, ftf::transform_frame_enu_ecef(Eigen::Vector3d(pos_enu.translation()), map_origin));
	...
	void vision_cb(const geometry_msgs::PoseStamped::ConstPtr &req)
	{
		...
		send_fake_gps(req->header.stamp, ftf::transform_frame_enu_ecef(Eigen::Vector3d(pos_enu.translation()), map_origin), req->pose);
